...
namespace App\Livewire\Cita\ReporteCita;
...
class ReporteCitaLivewire extends Component
{
    ...
    private function loadRankingUsuarios()
    {
        ...
        $this->rankingUsuarios = [
            'labels' => $data->map(fn($i) => User::find($i->gestor_id)?->name ?? "Usuario"),
            'data'   => $data->pluck('total'),
        ];
    }
    ...
    {
        return view('livewire.cita.reporte-cita.reporte-cita-livewire');
    }
}
