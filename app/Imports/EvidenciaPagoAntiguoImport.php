...
class EvidenciaPagoAntiguoImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $rows)
    {
        DB::transaction(function () use ($rows) {
            ...
            // 2️⃣ IDs existentes en BD
            $idsExistentes = EvidenciaPagoAntiguo::whereIn('id', $idsExcel)
                ->pluck('id')
                ->values();

            // ⛔ Si hay duplicados → abortar TODO
            if ($idsExistentes->isNotEmpty()) {
                throw ValidationException::withMessages([
                    'archivo' => [
                        'Ya existen registros con los siguientes ID: ' .
                            $idsExistentes->implode(', ')
                    ],
                ]);
            }

            // 3️⃣ Preparar data para insert masivo
            $data = [];

            foreach ($rows as $row) {

                if (empty($row['id']) || empty($row['imagen_url'])) {
                    continue;
                }
                ...
                $data[] = [
                    ...
                    'estado_evidencia_pago_id' => $row['estado_evidencia_pago_id'] ?? 1,
                    'estado_registro'       => $row['estado_registro'] ?? 'PENDIENTE',
            ...
            // 4️⃣ Insert masivo (si falla, rollback automático)
            if (!empty($data)) {
                EvidenciaPagoAntiguo::insert($data);
            }
        });
    }
